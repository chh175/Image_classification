...
##    images_placeholder = tf.placeholder(tf.float32, shape=(None,FLAGS.image_size,FLAGS.image_size,3), name='images')
##    labels_placeholder = tf.placeholder(tf.int64, shape=(None,FLAGS.num_classes), name='labels')
#    learning_rate_placeholder = tf.placeholder(tf.float32, name='learning_rate')
#    
#
#    with tf.device('/cpu:0'):
#        images, onehot_labels = TFrecord_dataset_input(True)            
#
#
#
#
...
#            )       
#    regularization_losses = tf.get_collection(tf.GraphKeys.REGULARIZATION_LOSSES)    
#    total_loss = tf.add_n([cross_entropy_loss] + regularization_losses, name='total_loss')
#
#
#
#    #学习率和优化器
#    learning_rate = _configure_learning_rate(learning_rate_placeholder, FLAGS.train_image_size, global_step)
...
#        
#  
#        if FLAGS.finetune_from_imagenet:                            
#            # 加载Imagenet预训练的权重
#            finetune_ckpt_init(sess)      
#            #global_step设置为0      
#            sess.run(tf.assign(global_step, 0))
...
#            epoch = step // steps_of_1_epoch
#            
#            train(sess, epoch, 
#                  learning_rate_placeholder, phase_train_placeholder,  global_step,
#                  total_loss, train_op, regularization_losses, steps_of_1_epoch)
#
#            print('validation running...')
#
...
#      learning_rate_placeholder, phase_train_placeholder, global_step, 
#      loss, train_op, regularization_losses, epoch_size):
#
#    batch_count = 0
#    
#    if FLAGS.init_learning_rate > 0.0:
#        lr = FLAGS.init_learning_rate
#    else:
#        lr = get_learning_rate_from_file(FLAGS.learning_rate_schedule_file, epoch)
#
#
...
#              (epoch, batch_count + 1, epoch_size, duration, err, np.sum(reg_loss)))
#        
#        batch_count += 1
#        
#    return step
...
##    parser.add_argument('--finetune_checkpoint', type=str, default='checkpoint/mobilenet_v2_1.0_224.ckpt')
##    parser.add_argument('--save_model_dir', type=str, default='flower_train')
##    parser.add_argument('--dataset_dir', type=str, default='flower_TFrecord')
##    parser.add_argument('--model_name', type=str, default='mobilenet_v2')
##    parser.add_argument('--finetune_exclude_scopes', type=str, default='MobilenetV2/Logits')
...
##    parser.add_argument('--num_epochs_per_decay', type=int, default=4)
##    parser.add_argument('--train_imagenet_size', type=int, default=6149)
